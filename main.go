...
func main() {
	for {
		GenerateMessageOptional(100)
		GenerateMessageValue(100)
	}
}
...
func GenerateMessageValue(n int) []*MessageValue {
	out := make([]*MessageValue, 0, n)
	for i := 0; i < n; i++ {
		out = append(out, &MessageValue{
			...
			Address: &AddressValue{
				...
				Random: &RandomValue{
					...
					NestedRandom: &NestedRandomValue{
						FieldA: randString(10),
						FieldB: randString(10),
						FieldC: randString(10),
						FieldD: randString(10),
...
func GenerateMessageOptional(n int) []*MessageOptional {
	var (
		Name     = randString(10)
		BirthDay = rand.Int63n(5)
		Phone    = randString(10)
	...
	out := make([]*MessageOptional, 0, n)
	for i := 0; i < n; i++ {
		out = append(out, &MessageOptional{
			Name:     &Name,
			BirthDay: &BirthDay,
			Phone:    &Phone,
			Siblings: &Siblings,
			...
			Address: &AddressOptional{
				...
				Random: &RandomOptional{
					...
					FieldY: &FieldY,
					FieldZ: &FieldZ,
					NestedRandom: &NestedRandomOptional{
						FieldA: &FieldA1,
						FieldB: &FieldB1,
						FieldC: &FieldC1,
						FieldD: &FieldD1,
